...
| `lcm_describe` | Inspect the current-session DAG by default, or pass `session_id` to inspect a known node/DAG in another LCM session; summary-node metadata includes `producer_model` and `escalation_level`. A node must belong to the requested session. Previews an `externalized_ref` without loading full content; `externalized_ref` remains current-session only and cannot be combined with `session_id`. |
...
| `lcm_inspect` | Read-only operator inventory for current-session lineage, message/frontier metadata, fresh tail, summary-node `producer_model`/`escalation_level`, externalized refs/readability, compaction skip/no-op reasons, and matched ignore/stateless patterns. It returns metadata only; use `lcm_load_session`/`lcm_expand` when you need content. |
...
the tools remove matching session ids before candidate caps, fusion, and final
ranking. Excluding the only session selected by `session_scope='current'` or
`'session'` returns a normal bounded response with no results.
