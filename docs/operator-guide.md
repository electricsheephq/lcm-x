...
| `LCM_FRESH_TAIL_PRESSURE_YIELD_MIN_OBSERVATIONS` | `3` | Consecutive tail-blocked engine invocations (preflight or compress) under host-observed pressure before the yield engages; an invocation that finds the fresh tail is not the blocker resets the count, and the count is relieved only when both the engine estimate and the host-observed token count fall under threshold; `1` yields on first observation |
