...
checks. Return `STATE_DRIFT` if identity or the pinned state changes while assessing it.

## 2. Decide Direct Landability
...
- each reviewer's latest opinionated review, exact reviewed commit, and protected CODEOWNER
  coverage;
- every review thread, the accepted issue, and any required independent semantic-review receipt.
...
Read the full diff and accepted issue. Verify lossless-data, provenance, Hermes compatibility,
backward-compatible defaults, tests, dependencies, and the issue's acceptance criteria. Require
an independent exact-head semantic review for governance, security, data integrity, migration,
compaction, persistence, lifecycle/session identity, or Hermes host-contract changes.

Require every verified finding to have a terminal disposition and every thread to be resolved.
Do not count the PR author, CI, this skill, or a flat bot comment as independent review.
...
- `OWNER_GATE`: required authority, issue acceptance, ownership, or trusted policy is unavailable;
- `STATE_DRIFT`: repository, base, head, or other evaluated identity changed.

Include the PR/head, protected base/ruleset, matched trusted pairs, review/thread summary, issue,
blocker codes, finding dispositions, and proof boundary. Even `READY_FOR_AUTHORIZED_LANDING` is
read-only advice. Hand it to `land-pr` only after a maintainer separately authorizes merging PR N
at exact head H.

## Failure Behavior

Fail closed to `OWNER_GATE` or `STATE_DRIFT` when live evidence is missing or changes. Never
repair, push, approve, mutate metadata, enable auto-merge, bypass a gate, or transform a readiness
request into a landing request.
