...
def test_filter_exclusion_scan_pages_to_the_covered_end(
    ...
    def paged_rows(conversation_id, after_store_id=0, limit=10_000, **kwargs):
        assert conversation_id == "issue-5-filter-pages"
        calls.append((after_store_id, limit))
        return first_page if after_store_id == 0 else last_page

    monkeypatch.setattr(engine._store, "get_conversation_messages_after", paged_rows)
    try:
