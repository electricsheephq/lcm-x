...
def candidate(tmp_path, monkeypatch):
    cfg = LCMConfig(
        database_path=str(tmp_path / "lcm.db"), native_publication_fallback=True,
...
def test_rejected_publication_recovers_without_claiming_coverage(candidate, monkeypatch):
    calls = install_native(monkeypatch)
    # Stored old history is absent from the active replay. The normal engine
    # must reject a new leaf which tries to leap over these durable rows.
    ...
    assert candidate.last_compression_status == "host_fallback"
...
def test_failed_recovery_preserves_exact_input(candidate, monkeypatch, kind):
    ...
    result = candidate._native_after_publication_conflict(original)
...
def test_no_native_dispatch_without_admission(candidate, monkeypatch, mode):
    calls = install_native(monkeypatch)
    if mode == "disabled":
        candidate._config.native_publication_fallback = False
    elif mode == "unfenced":
        candidate._compression_cancelled_check = None
    else:
        candidate._compression_cancelled_check = lambda: True
    messages = history()
    assert candidate._native_after_publication_conflict(messages) is messages
    assert not calls


def test_config_opt_in(monkeypatch):
    monkeypatch.delenv("LCM_NATIVE_PUBLICATION_FALLBACK", raising=False)
    assert LCMConfig.from_env().native_publication_fallback is False
    monkeypatch.setenv("LCM_NATIVE_PUBLICATION_FALLBACK", "true")
    assert LCMConfig.from_env().native_publication_fallback is True
