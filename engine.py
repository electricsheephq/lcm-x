...
class LCMEngine(
    ...
    def _replay_scaffold_layout(
        ...
        """Return live positional scaffold flags and replay identities (#488)."""
    ...
    def _stored_replay_identities(
        ...
        """Return full stored identities; stored position is not provenance."""
        return [
    ...
    def _ingest_messages(
        ...
        *,
        ...
        if (
            ...
            native_lossy = proof.get("native") and any(_has_lossy_redacted_identity(identity) for identities in (proof.get("output"), host_input) for identity in identities or ())
            if native_lossy:
                self._ingest_cursor_needs_reconcile = True
