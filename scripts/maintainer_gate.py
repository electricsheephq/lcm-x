...
def evaluate(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if mode not in {"readiness", "landing", "post_merge"}:
        blockers = ["MODE_UNSUPPORTED"]
        return _receipt(data, "OWNER_GATE", blockers, [], "unknown")
    ...
    if mode == "landing":
        ...
        elif data.get("merge_authorization", {}).get("use_admin_bypass") not in (None, False):
...
def _evaluate_post_merge(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if data.get("repository") != REPOSITORY:
        blockers.append("REPOSITORY_MISMATCH")
    if policy.get("source") != "protected-main" or not policy.get("ruleset_id"):
        blockers.append("PROTECTED_POLICY_UNTRUSTED")
