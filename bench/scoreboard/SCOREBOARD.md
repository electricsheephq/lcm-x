...
- ⚠ FROZEN vs current main (2026-08-26): measured pre-v0.23.1 privacy trio; the production
  `lcm_recall` arm may not be compared against any post-v0.23.1 production-path row until the
  registered re-bank (see the reproducibility note in FINDING-F53 + the privacy-trio boundary
  row in bench/BASELINE-LEDGER.md). Reproducibility on current main is UNVERIFIED, not proven
  broken.
