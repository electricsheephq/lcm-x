...
| #332/#333/#338 cloud-embedding + durable redaction (privacy trio, v0.23.1) | 2026-08-23 | **recall-path content shaping**: redaction changes what text is embedded (cloud) AND stored (durable messages/FTS/summaries when patterns enabled) — see #367 | **cross-boundary comparisons FROZEN**: any V1-M/LoCoMo row embedded post-v0.23.1 is a different corpus than F53's; requires a fresh re-bank with this boundary noted before comparison. Ledger row was MISSING at merge (reconciled 2026-08-26 after the architect's interim-work adversarial review) | OPEN |
| #365 PEM-redaction ordering fix (security) | 2026-08-26 | recall-path content shaping (fixes a key-body leak; changes embedding transform ⇒ privacy revision bumped v1→v2) | vectors re-embed under the new revision by design; folded into the same privacy-trio re-bank boundary above | OPEN (with #332 trio) |
